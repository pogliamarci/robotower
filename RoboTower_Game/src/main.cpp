...
int main(int argc, char **argv)
{
	...
	GameConfiguration configuration = ConfigurationFactory::loadConfiguration("/../robotower.xml");
	...
	QObject::connect(&app, SIGNAL(aboutToQuit()), &rosPublisher,
			SLOT(quitNow()));
	QObject::connect(&app, SIGNAL(aboutToQuit()), &gameControl,
			SLOT(quitNow()));
	QObject::connect(&rosPublisher, SIGNAL(rosQuits()), &app, SLOT(quit()));

	/* signals from the controller to the GUI */
	QObject::connect(&gameControl, SIGNAL(timeIsUpdated(int)), &mainWindow,
			SLOT(updateTime(int)));
	QObject::connect(&gameControl, SIGNAL(pointsAreUpdated(int)), &mainWindow,
			SLOT(updatePoints(int)));
	...
	QObject::connect(&gameControl, SIGNAL(endGame(int,int,int)), &mainWindow,
			SLOT(updateHistory(int,int,int)));
	QObject::connect(&gameControl, SIGNAL(towersUpdate(int, int)), &mainWindow,
			SLOT(updateTowers(int, int)));

	/* signals between ROS and the controller */
	QObject::connect(&rosPublisher, SIGNAL(towersUpdate(int)), &gameControl,
			SLOT(updateTowers(int)));
	QObject::connect(&gameControl, SIGNAL(robotIsEnabled(bool)), &rosPublisher,
			SLOT(enableIsaac(bool)));
	...
	QObject::connect(&mainWindow, SIGNAL(start()), &gameControl,
			SLOT(startGame()));
	QObject::connect(&mainWindow, SIGNAL(stop()), &gameControl,
			SLOT(stopGame()));
	QObject::connect(&mainWindow, SIGNAL(togglePause()), &gameControl,
			SLOT(togglePause()));
	QObject::connect(&mainWindow, SIGNAL(newGame()), &gameControl,
			SLOT(resetGame()));

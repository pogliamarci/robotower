...
operator== (const PointClass & first, const PointClass & second)
{
	return (first.getR () == second.getR () &&
		first.getG () == second.getG () && first.getB () == second.getB ());
}
...
void
ColorClassifier::Create_Matrix (ColorCompressionType CCF)
{
	ColorCompressionFactor = CCF;
	ColorMatrix = (unsigned char ***) malloc ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) * sizeof (unsigned char **));
	ColorMatrixYUV = (unsigned char ***) malloc ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) * sizeof (unsigned char **));
	for (unsigned int i = 0; i < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); i++)
	{
		ColorMatrix[i] =(unsigned char **)malloc ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) *sizeof (unsigned char *));
		ColorMatrixYUV[i] =(unsigned char **)malloc ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) *sizeof (unsigned char *));
		for (unsigned int j = 0; j < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); j++)
			{
				ColorMatrix[i][j] = (unsigned char *) malloc ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) * sizeof (unsigned char));
				ColorMatrixYUV[i][j] = (unsigned char *) malloc ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) * sizeof (unsigned char));
			}
	}

	for (unsigned int r = 0; r < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); r++)
	for (unsigned int g = 0; g < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); g++)
			for (unsigned int b = 0; b < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); b++)
				{
					ColorMatrix[r >> ColorCompressionFactor][g >> ColorCompressionFactor][b >> ColorCompressionFactor] = 'U';
					ColorMatrixYUV[r >> ColorCompressionFactor][g >> ColorCompressionFactor][b >> ColorCompressionFactor] = 'U';
				}
}
	
			
void
ColorClassifier::Delete_Matrix ()
{
	for (unsigned int i = 0; i < ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) >> ColorCompressionFactor); i++)
	{
		for (unsigned int j = 0; j < ((LEVELS_PER_CHANNEL >> ColorCompressionFactor) >> ColorCompressionFactor); j++)
		{
...
void
ColorClassifier::save_matrix (char *filename)
{
...
void
ColorClassifier::load_matrix (const char *filename)
{
	char trash[256];
	int ccf;
	...
	for (unsigned int y = 0; y < LEVELS_PER_CHANNEL; y++)
		for (unsigned int u = 0; u < LEVELS_PER_CHANNEL; u++)
			for (unsigned int v = 0; v < LEVELS_PER_CHANNEL; v++)
				{
					//	  printf("%d, %d, %d ",y,u,v);
					pixel32 = ColorSample::yuv2rgb((unsigned char)y,(unsigned char)u, (unsigned char)v);
					//	  printf(" %d ",pixel32);
					//	  printf("%d, %d, %d\n",pixel[0], pixel[1], pixel[2]);
					ColorMatrixYUV[y>>ColorCompressionFactor][u>>ColorCompressionFactor][v>>ColorCompressionFactor] 
					= ColorMatrix[pixel[0]>>ColorCompressionFactor][pixel[1]>>ColorCompressionFactor][pixel[2]>>ColorCompressionFactor];
				}
}

void
DbscanColorClassifier::build (ColorDataset * Dataset, int Eps,
			      unsigned int NEps)
{
	...
	for (SamplesIterator = Dataset->begin ();
	SamplesIterator != Dataset->end (); SamplesIterator++)
	{
...
void
DbscanColorClassifier::fast_build (ColorDataset * Dataset, int Eps,
                                unsigned int NEps, bool SEEDS)
{
	...
	SamplesIterator != CompressedColorSamples.end (); SamplesIterator++)
	{
		...
		NearIterator != CompressedColorSamples.end (); NearIterator++)
		{
			...
			RDist =
				abs ((int)
				((unsigned int) NearIterator->
					getR () >> ColorCompressionFactor -
				(unsigned int) SamplesIterator->
				getR () >> ColorCompressionFactor));
			GDist =
				abs ((int)
				((unsigned int) NearIterator->
					getG () >> ColorCompressionFactor -
				(unsigned int) SamplesIterator->
				getG () >> ColorCompressionFactor));
			BDist =
				abs ((int)
				((unsigned int) NearIterator->
					getB () >> ColorCompressionFactor -
				(unsigned int) SamplesIterator->
				getB () >> ColorCompressionFactor));
		...
		if (count >= NEps)
		{
			...
			if (startR < 0)
				startR = 0;
			else if (endR >=(int) (LEVELS_PER_CHANNEL >> ColorCompressionFactor))
				endR = (LEVELS_PER_CHANNEL >> ColorCompressionFactor) - 1;

			if (startG < 0)
				startG = 0;
			else if (endG >=(int) (LEVELS_PER_CHANNEL >> ColorCompressionFactor))
					endG = (LEVELS_PER_CHANNEL >> ColorCompressionFactor) - 1;

			if (startB < 0)
				startB = 0;
			else if (endB >=(int) (LEVELS_PER_CHANNEL >> ColorCompressionFactor))
			...
			CompressedColorSamples.erase (SamplesIterator);
	}	//fine tutto ciclo
	buildYUV();
		//fine metodo 
};
...
void
KnnColorClassifier::build (ColorDataset * Dataset, int MaxDistance, unsigned int K)
{
	int SampleDistance = 0, RDist = 0, GDist = 0, BDist = 0;
...
void
KnnColorClassifier::fast_build (ColorDataset * Dataset, int MaxDistance,
                          unsigned int K, bool SEEDS)
{
	...
	{
		DataPoints[Index][0] =
		((unsigned short int) SamplesIterator->
			getR () >> ColorCompressionFactor);
		DataPoints[Index][1] =
		((unsigned short int) SamplesIterator->
			getG () >> ColorCompressionFactor);
		...
		OutputClasses[PointClass
				((short int)DataPoints[Index][0], (short int)DataPoints[Index][1],
				(short int)DataPoints[Index][2])] = SamplesIterator->getClass ();
		Index++;
	}
	...
	KccTree =
	new ANNkd_tree (DataPoints, CompressedColorSamples.size (),
			POINT_DIMENSIONS);
		...
		g < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); g++)
		{
			...
			b < (LEVELS_PER_CHANNEL >> ColorCompressionFactor); b++)
			{
				...
				for (Index = 0; Index < K; Index++)
				{
					...
					{
						pPointClass =
						new PointClass ((short int)DataPoints[NnIdx[Index]][0],
								(short int)DataPoints[NnIdx[Index]][1],
								(short int)DataPoints[NnIdx[Index]][2]);
						OutputClassesIterator =
						OutputClasses.find (*pPointClass);

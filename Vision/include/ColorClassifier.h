...
typedef enum{NO_COMPRESSION,COMPRESSION_TWO,COMPRESSION_FOUR,COMPRESSION_EIGHT,COMPRESSION_SIXTEEN} ColorCompressionType;
...
const unsigned int CELLS_PER_COLOR = LEVELS_PER_CHANNEL << COLOR_COMPRESSION_FACTOR;
...
class PointClass
{
	...
	public:
		PointClass():R(0),G(0),B(0){};
		PointClass(short int RR,short int GG, short int BB):R(RR),G(GG),B(BB){};
		short int getR() const { return R; }
		short int getG() const { return G; }
		short int getB() const { return B; }
};
...
class ColorClassifier
{
	...
	*/
	...
	void Delete_Matrix();
	ColorCompressionType Get_CCF(){return ColorCompressionFactor;}
	...
	*/
	unsigned char get_color(unsigned int R, unsigned int G, unsigned int B, ColorSpace CS=RGB24);
...
class DbscanColorClassifier : public ColorClassifier
{
	...
	*/
	DbscanColorClassifier ( ) : ColorClassifier ( ) { }
	...
	*/
	DbscanColorClassifier (ColorCompressionType CCF) : ColorClassifier( CCF) { }
...
{  
	...
	*See the Classifier costructor for more details.
	*/
	KnnColorClassifier ( ) : ColorClassifier ( ) { } 
	...
	* @param CCF is the value of the color compression factor 
	*/
	KnnColorClassifier (ColorCompressionType CCF) : ColorClassifier( CCF) { }
...
inline unsigned char ColorClassifier::get_color(unsigned int R, unsigned int G, unsigned int B, ColorSpace CS)
{
	if(CS==RGB24)
	 	return ColorMatrix[R >> ColorCompressionFactor][G >> ColorCompressionFactor][B >> ColorCompressionFactor];
	else if(CS==YUV411)
		return ColorMatrixYUV[R >> ColorCompressionFactor][G >> ColorCompressionFactor][B >> ColorCompressionFactor];
	else
		return 'U';
};
